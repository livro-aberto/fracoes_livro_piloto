...
\end{itemize} %s
 \vspace{.1cm}
  ...
  \begin{itemize} %s
    ...
    \item Cada grupo deve receber os oito retângulos recortados, disponíveis para reprodução no final do livro.
...
\item       Segundo a avaliação do professor, a atividade pode ser realizada em duas etapas. Em um primeiro momento, os alunos recebem as primeiras quatro das oito imagens e realizam a atividade com essas imagens - cuja comparação se dá apenas pela sobreposição. Em seguida, recebem as outras quatro, para concluir a atividade. 
   ...
   \item Nossa experiência na implementação desta atividade em sala de aula mostra uma situação de catarse quando os estudantes percebem que formas diferentes podem representar a mesma fração da unidade. Uma sugestão é colocar os retângulos no quadro e solicitar aos estudantes que criem novas equipartições que representem um quarto do retângulo.
\end{itemize} %s
