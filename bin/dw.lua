...
wrapkey      <- "professor" / "exercicio" / "introdutorio" / "resposta" / "imagem" / "figura"
                / "abstrato" / "conexoes" / "explorando" / "massa"
eqblock      <- "\begin{equation}" newline (%s* !"\end{equation}" [^%nl]+ newline)+ "\end{equation}" newline
...
file2 = io.open ("header.tex", "r")
outstring = file2:read("*a")
io.close(file2)
...
outstring = outstring .. [=[\end{document}]=]
