...
local link = P('[[') * (known - S(']|'))^0 * P('|') * token('link', (known - P(']'))^0) * P(']]')
...
local atividade = token('atividade', P('==== - Atividade ====') + P('==== Atividade - ===='))
local title = P('===== ') * token('title', simpletext) * P('=====')
local titlechapter = P('====== ') * harmless^-12 * token('titlechapter', simpletext) * P('======')
local titleless = P('==== ') * token('title', simpletext) * P('====')
local include = P('{{page>') * token('include', simpletext) * P('}}')
local image = P('{{') * S(':|')^0 * token('image', simpletext) * P('|')^0 * simpletext^0 * P('}}')
...
local decoline = link + bold + under + italic + mono + quote + supermath + image + simplemath + newline + token('simple', simpletext)
